...
/* fork addition: the custom-profile subscription cache dir was renamed
 * from "custom/subscriptions" to the hidden "custom/.subscriptions" so
 * it no longer shows up in the "Upload Profile" file browser. Move any
 * already-cached subscription files over once, so users upgrading from
 * an older build of this fork don't lose their cache. */
